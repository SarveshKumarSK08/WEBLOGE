...
const PostCard = ({ posts }) => {
  ...
  return (
    <>
     {posts.map((post) => (
        <div key={post.node.title} className="bg-white shadow-lg rounded-lg p-0 lg:p-8 pb-12 mb-8">
          ...
          <Image
              unoptimized
              src={post.node.authors[0].photo.url}
              alt={post.node.authors[0].name}
              height={30}
              width={30}
              className="align-middle rounded-full"
            />
        <p className="inline align-middle text-gray-700 ml-2 font-medium text-lg">{post.node.authors[0].name}
